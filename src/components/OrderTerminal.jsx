...
export default function OrderTerminal({
  ...
  // Internal panel view: 'main' (ordering) | 'bill'
  ...
  const [paymentChoice, setPaymentChoice] = useState(null)  // 'cash'|'card'|'upi' or null
  const [completedOrder, setCompletedOrder] = useState(null)

  // When the table prop transitions to 'billed' (e.g. we opened a previously-billed table),
  // auto-build a bill from current data so we can render the bill view.
  useEffect(() => {
    ...
    <div className="fixed inset-0 z-40">
      ...
      >
        ...
        <header className={cls(
          'px-6 py-5 border-b border-sand/60 flex items-center justify-between',
          s.cardBg,
        )}>
      ...
      {paymentChoice && bill && !completedOrder && (
        <PaymentConfirmModal
          ...
          onConfirm={async () => {
            await handleCheckout(paymentChoice)
          }}
        />
      )}
