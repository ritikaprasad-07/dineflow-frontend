import { RefreshCw, Settings, LayoutGrid, BarChart3, LogOut } from 'lucide-react'
import { cls } from '../lib/format'

export default function TopNav({
  view, onChangeView, onOpenSetup, onRefresh, onLogout, refreshing, totalTables,
}) {
  return (
    <header className="border-b border-sand/60 bg-cream/70 backdrop-blur-md sticky top-0 z-30">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-ivory border border-sand rounded-full p-1">
          <NavTab active={view === 'tables'} icon={<LayoutGrid size={15} />} label="Floor" onClick={() => onChangeView('tables')} />
          <NavTab active={view === 'analytics'} icon={<BarChart3 size={15} />} label="Analytics" onClick={() => onChangeView('analytics')} />
        </nav>
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onRefresh} className="btn-ghost" title="Refresh" aria-label="Refresh">
            <RefreshCw size={16} className={cls(refreshing && 'animate-spin')} />
          </button>
          <button onClick={onOpenSetup} className="btn-ghost" title="Setup" aria-label="Setup">
            <Settings size={16} />
          </button>
          {onLogout && (
            <button
              onClick={onLogout}
              className="btn-ghost hover:text-clay-700"
              title="Log out"
              aria-label="Log out"
            >
              <LogOut size={16} />
  ...
  return (
    <button
      ...
      className={cls(
        ...
        active ? 'bg-ink text-cream shadow-sm' : 'text-umber hover:text-ink'
      )}
    >
      {icon}
      {label}
